  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: ListView(shrinkWrap: true, children: <Widget>[
        createStatsTable(),
        SimpleBarChart(_createBarChart("Runs"),
            animate: true, title: "Runs per Game"),
        SimpleLineChart(_createLineData("Batting Average"),
            animate: true, title: "Batting Average Progression"),
        SimpleBarChart(_createBarChart("Strike Rate"),
            animate: true, title: "Strike Rate Frequency"),
        DonutPieChart(
          _createDonutData("Not Out"),
          animate: true,
          title: "Not Out vs Dismissals",
          subtitle: "0 is Dismissals and 1 is Not Outs",
        ),
  ...
  List<LineChartData> generateBattingAverage() {
    ...
    for (int i = 0; i < statistics.length; i++) {
      if (i == 0) {
        ...
        if (statistics[i].ballsFaced != 0) {
          if (statistics[i].notOut != 0) {
            dismissals.add(dismissals[
                i - 1]); //Copies the previous total dismissals to this index
          } else {
            dismissals.add(
                dismissals[i - 1] + 1); //Add the previous total dismissals by 1
          }
        } else {
    ...
    for (int i = 0; i < statistics.length; i++) {
      ...
      if (dismissals[i] == 0 || totalRuns[i] == 0) {
        //Checks if the person was out or if they scored 0
        average = totalRuns[i]
            .toDouble(); //Their average for that game will simply be the runs they scored
      } else {
        average = totalRuns[i] /
            dismissals[
                i]; //Their average will be divided by the total dismissals
      }
  ...
  List<charts.Series<BarChartData, String>> _createBarChart(String type) {
    ...
    if (type == "Runs") {
      ...
      for (int i = 0; i < statistics.length; i++) {
        ...
        list.add(
            BarChartData(statistics[i].name, statistics[i].runs, barColors[j]));
        j++;
      }
        ...
        itemBuilder: (BuildContext ctxt, int index) {
          return InkWell(
              ...
              child: CustomStatisticCard(
                  object: statistics[index], width: width, type: 0));
        });
  }
