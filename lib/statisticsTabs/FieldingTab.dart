...
class _MyFieldingTabState extends State<FieldingTab> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: ListView(children: <Widget>[
        createStatsTable(),
        SimpleSeriesLegend(_createFieldingChart('Taken', 'Dropped', "Catches"),
            animate: true, title: "Catches Taken vs. Dropped"),
        SimpleSeriesLegend(_createFieldingChart('Taken', 'Missed', "Run Outs"),
            animate: true, title: "Successful vs. Missed Run Outs"),
        SimpleSeriesLegend(_createFieldingChart('Taken', 'Missed', "Stumpings"),
            animate: true, title: "Successful vs. Missed Stumpings"),
        //Add datatable here
        statList(),
      ]),
  ...
  generateFieldingStats() {
    ...
    for (int i = 0; i < statistics.length; i++) {
      ...
      totalStumpings +=
          (statistics[i].stumpings + statistics[i].stumpingsMissed);
  ...
  List<charts.Series<SeriesChartData, String>> _createFieldingChart(
      String firstKey, String secondKey, String type) {
    List<SeriesChartData> takenList;
    List<SeriesChartData> missedList;

    if (type == "Catches") {
      takenList = statistics
          .map((stat) => SeriesChartData(stat.name, stat.catches))
          .toList();
      missedList = statistics
          .map((stat) => SeriesChartData(stat.name, stat.catchesMissed))
          .toList();
    } else if (type == "Run Outs") {
      takenList = statistics
          .map((stat) => SeriesChartData(stat.name, stat.runOuts))
          .toList();
      missedList = statistics
          .map((stat) => SeriesChartData(stat.name, stat.runOutsMissed))
          .toList();
    } else if (type == "Stumpings") {
      takenList = statistics
          .map((stat) => SeriesChartData(stat.name, stat.stumpings))
          .toList();
      missedList = statistics
          .map((stat) => SeriesChartData(stat.name, stat.stumpingsMissed))
          .toList();
    }
        ...
        itemBuilder: (BuildContext ctxt, int index) {
          return InkWell(
              ...
              child: CustomStatisticCard(
                  object: statistics[index], width: width, type: 2));
        });
  }
