...
class DatabaseHelper {
  ...
  Future<List<JournalInformation>> getJournals() async {
    ...
    return List.generate(maps.length, (i) {
      ...
      return JournalInformation(
          maps[i][columnJournalName],
          maps[i][columnDetails],
          maps[i][columnDate],
          maps[i][columnJournalId]);
    });
  }
  ...
  Future<int> deleteJournal(int id) async {
    final db = await database;
    return await db
        .delete(tableJournals, where: '$columnJournalId = ?', whereArgs: [id]);
  }
  ...
  Future<StatisticInformation> queryStatistics(int id) async {
    ...
    List<Map> maps = await db.query(tableStatistics,
        columns: [
          columnStatisticsId,
          columnStatisticsName,
          columnStatisticsRuns,
          columnStatisticsBallsFaced,
          columnStatisticsNotOut,
          columnStatisticsWickets,
          columnStatisticsOvers,
          columnStatisticsRunsConceeded,
          columnStatisticsRunOuts,
          columnStatisticsCatches,
          columnStatisticsStumpings,
          columnStatisticsRunOutsMissed,
          columnStatisticsCatchesMissed,
          columnStatisticsStumpingsMissed
        ],
  ...
  Future<List<StatisticInformation>> getStatistics() async {
    ...
    return List.generate(maps.length, (i) {
      ...
      return StatisticInformation(
          ...
          maps[i][columnStatisticsRuns],
          maps[i][columnStatisticsBallsFaced],
          maps[i][columnStatisticsNotOut],
          maps[i][columnStatisticsWickets],
          maps[i][columnStatisticsOvers],
          maps[i][columnStatisticsRunsConceeded],
          maps[i][columnStatisticsRunOuts],
          maps[i][columnStatisticsCatches],
          maps[i][columnStatisticsStumpings],
          maps[i][columnStatisticsRunOutsMissed],
          maps[i][columnStatisticsCatchesMissed],
          maps[i][columnStatisticsStumpingsMissed],
          maps[i][columnStatisticsId]);
    });
  }
  ...
  Future<void> updateStatistics(
      StatisticInformation statistic, int index) async {
    // Get a reference to the database.
    final db = await database;
  ...
  Future<int> deleteStatistic(int id) async {
    final db = await database;
    return await db.delete(tableStatistics,
        where: '$columnStatisticsId = ?', whereArgs: [id]);
  }
}
